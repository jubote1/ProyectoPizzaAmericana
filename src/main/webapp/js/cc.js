...
$(document).ready(function() {
    ...
    dtpedido = $('#grid-encabezadopedido').DataTable( {
            ...
            { "mData": "tiempopedido"  },
            {
                "mData": "accion",
                className: "center",
                defaultContent: '<input type="button" class="btn btn-default btn-xs" onclick="EliminarPedido()" value="Eliminar"></button>'
            },
...
function consultarPedido() 
{
	...
	$.getJSON(server + 'ConsultaIntegradaPedidosEnCurso?fechainicial=' + fechaini +"&fechafinal=" + fechafin + "&tienda=" + tienda +  "&numeropedido=" + numpedido + "&estado=" + estado+"&estadotienda=" + estadoTienda, function(data1){
	                		
	                		table.clear().draw();
							for(var i = 0; i < data1.length;i++){
								//Cambio para poner acción al botón
								var cadaDetalle = data1[i];
								table.row.add( {
										"idpedido": cadaDetalle.idpedido,
										"tienda": cadaDetalle.tienda,
								        "fechainsercion": cadaDetalle.fechainsercion,
								        "cliente": cadaDetalle.cliente,
								        "direccion": cadaDetalle.direccion,
								        "telefono": cadaDetalle.telefono,
								        "totalneto": cadaDetalle.totalneto,
								        "estadopedido": cadaDetalle.estadopedido,
								        "usuariopedido": cadaDetalle.usuariopedido,
								        "enviadopixel": cadaDetalle.enviadopixel,
								        "estadoenviotienda": cadaDetalle.estadoenviotienda,
								        "numposheader": cadaDetalle.numposheader,
								        "formapago": cadaDetalle.formapago,
								        "tiempopedido": cadaDetalle.tiempopedido,
								        "accion":'<button type="button" class="btn btn-danger btn-xs" onclick="EliminarPedido(' + cadaDetalle.idpedido + ')">Eliminar</button>',
								        "idtienda": cadaDetalle.idtienda,
								        "urltienda": cadaDetalle.urltienda,
								        "stringpixel": cadaDetalle.stringpixel,
								        "idlink": cadaDetalle.idlink,
								        "fechafinalizacion": cadaDetalle.fechafinalizacion,
								        "fechapagovirtual": cadaDetalle.fechapagovirtual
								    	}).draw();
							}
...
function EliminarPedido(idPedidoEli)
{
	var resultado;
	$.confirm({
							    				...
							    				success: function(data){ 
							    						...
							    						if (resultado == 'OK')
														{
															if (idPedido != 0)
															{
																...
																$.ajax({ 
												    				url: server + 'EliminarPedidoSinConfirmar?idpedido=' + idPedidoEli , 
												    				dataType: 'json', 
												    				async: false, 
												    				success: function(data){
												    					var resul =  data[0];
																	...
																	//Vamos a repintar los pedidos filtrados antes de eliminar
																	consultarPedido();
														}
														else
														{
