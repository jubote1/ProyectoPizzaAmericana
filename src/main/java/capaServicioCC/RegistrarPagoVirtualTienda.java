 ...
 */
@WebServlet("/RegistrarPagoVirtualTienda")
public class RegistrarPagoVirtualTienda extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RegistrarPagoVirtualTienda() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			double valorPagoVirtual = Double.parseDouble(request.getParameter("valorpagovirtual"));
			double valorPedido = Double.parseDouble(request.getParameter("valorpedido"));
			response.addHeader("Access-Control-Allow-Origin", "*");
			response.setContentType("application/json");
			PedidoCtrl pedCtrl = new PedidoCtrl();
			pedCtrl.registrarPagoVirtualTienda(idPedidoTienda, idTienda, valorPagoVirtual, valorPedido,idLink);
