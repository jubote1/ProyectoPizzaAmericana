...
public class SolicitudFacturaImagenesDAO {
	...
	public static int insertarSolicitudFacturaImagen(SolicitudFacturaImagenes solicitud)
	{
		...
		try
		{
			...
			String insert = "insert into solicitud_factura_imagenes (idsolicitud,rutaimagen) values ('" + solicitud.getIdSolicitud() + "', '" + solicitud.getRutaImagen() + "')" ; 
			logger.info(insert);
			stm.executeUpdate(insert, Statement.RETURN_GENERATED_KEYS);
			ResultSet rs = stm.getGeneratedKeys();
			if (rs.next()){
				idSolicitudPQRSImaIns =rs.getInt(1);
				logger.info("Id Solicitud insertada en bd " + idSolicitudPQRSImaIns);
	        }
			stm.close();
			con1.close();
		}
	...
	public static ArrayList<String> consultarSolicitudFacturaImagenes(int idSolicitud)
	{
		Logger logger = Logger.getLogger("log_file");
		...
		{
			Statement stm = con1.createStatement();
			String select = "select * from solicitud_factura_imagenes where idsolicitud = " + idSolicitud; 
			logger.info(select);
