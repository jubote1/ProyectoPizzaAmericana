 ...
 */
	...
	/**
	 * Realizamos replica del método obtenerTiendas,con el fin de realiar control para que se retornen tiendas funcionales, es decir 
	...
	public static ArrayList<Tienda> obtenerTiendasFuncionales()
	{
		...
		{
			Statement stm = con1.createStatement();
			String consulta = "select * from tienda WHERE hosbd <> ''";
			logger.info(consulta);
			ResultSet rs = stm.executeQuery(consulta);
			while(rs.next()){
				int idTienda = rs.getInt("idtienda");
