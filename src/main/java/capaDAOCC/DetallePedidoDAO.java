...
public class DetallePedidoDAO {
	...
	public static String retornarIdProductosSalesManago(int idPedido)
	{
		...
		{
			Statement stm = con1.createStatement();
			String consulta = "select a.idproducto, a.idespecialidad1, a.idespecialidad2 from detalle_pedido a, producto b  where a.idproducto = b.idproducto and b.tipo = 'PIZZA' and a.idpedido = " + idPedido;
			logger.info(consulta);
			ResultSet rs = stm.executeQuery(consulta);
			int idProducto;
			int idEspecialidad1;
			...
			while(rs.next()){
				...
				if(esPrimero)
				{
					respuesta = Integer.toString(idProducto)+"-"+Integer.toString(idEspecialidad1);
					esPrimero = false;
				}else
				{
					respuesta = respuesta +" , " +  Integer.toString(idProducto)+"-"+Integer.toString(idEspecialidad1);
				}
			}
			rs.close();
			stm.close();
